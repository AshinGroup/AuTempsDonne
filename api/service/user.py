...
class UserService:
    ...
    def insert_activity(self, user_id: int, activity_id: int) -> None:
        ...
        self.user_repo.insert_activity(
    ...
    def update(self, user_id: int, args: dict) -> None:
        update_user = User(first_name=args['first_name'], last_name=args['last_name'],
                           email=args['email'], phone=args['phone'], password=args['password'], status=args['status'])
        user = self.user_repo.select_one_by_id(user_id=user_id)
    ...
    def delete_activity(self, user_id: int, activity_id: int) -> None:
        ...
        if not activity_exist:
            raise UserParticipatesActivityNotFoundException(
                user_id=user_id, activity_id=activity_id)
        self.user_repo.delete_activity(
            user_id=user_id, activity_id=activity_id)
        ...
        if user.course:
            for course in user.course:
                if course.course_id == course_id:
                    course_exist = True
        if not course_exist:
            raise UserFollowsCourseNotFoundException(
