...
class CollectRepo():
    ...
    def select_one_by_id(self, collect_id: int) -> Collect:
        try:
            ...
            raise CollectAccessDbException(
                collect_id=collect_id, method="getting")

    def select_per_page(self, page: int) -> list[Collect]:
        try:
            collects = Collect.query.paginate(page=page, per_page=10)
            if not collects:
                return None
    ...
    def select_by_search(self, page: int, search: str) -> list[Collect]:
        try:
            collects = Collect.query.filter(Collect.name.like(
    ...
    def insert(self, new_collect: Collect, demands: list[int]) -> None:
        try:
            with app.app_context():
                ...
                to_add_demands = Demand.query.filter(
                    Demand.id.in_(demands)).all()
    ...
    def update(self, collect_id: int, update_collect: Collect) -> None:
        try:
            ...
            raise CollectAccessDbException(
                collect_id=collect_id, method="updating")
    ...
    def delete(self, collect_id: int) -> None:
        try:
            ...
            raise CollectAccessDbException(
                collect_id=collect_id, method="deleting")
